...
export const trafficQuestions = (where: string): AssistQuestion[] => [
  q("Explain traffic",
    ...
    "Use only traffic in the context: the last hour, from the sources it names (NetFlow, OneAgent).",
  ...
  q("What to fix first",
    ...
    "One bullet each: the finding, why it ranks there, the team it belongs to and the exact element (device, port, workload, site)."),
  q("What is missing",
